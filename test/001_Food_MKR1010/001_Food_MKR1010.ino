...
void loop() {
  ...
  const float spoilFrac = 0.40; // assume 40% increase ~ spoiled
  float progress = 0.0;
  if (spoilFrac > 0.0) {
...
// This is a simple heuristic model for the project:
// Base: ~72h (~3 days) at 18–26°C and 40–80% RH.
// Very warm / very cold / very humid / very dry → shorter shelf life.
float estimateMaxShelfHours(float t, float h) {
  float baseHours = 72.0; // default ~3 days

  // Temperature effect
  if (t > 30.0) {
    baseHours = 36.0; // very warm, spoils faster
  } else if (t > 26.0) {
    baseHours = 48.0;
  } else if (t < 18.0) {
    baseHours = 48.0; // cooler than ideal for bananas
  }
